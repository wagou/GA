...
void ga(int n,City *city,FILE* fp, char **map);
...
void ga(int n,City *city,FILE* fp, char **map) {
    ...
    int timelimit = 1200;
    int change = 0;
    while (time(NULL) - time1 < timelimit && gen - change < 100000) {
...
double solve(City *city, int n , int *route, int **gen_routes)
{
    ...
    for (int i = 0; i < 100; i++) { // 一世代当たりの標本数(初期解生成数)100
        ...
        double temp_d = search(city,n,temp_route,sum_d,0,0);
        double value = 1.0 / (temp_d * temp_d * temp_d); // 評価値は山登り法で求めた局所最短距離の逆数の3乗とする
        valuesum += value;
